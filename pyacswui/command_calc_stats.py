...
class CommandCalcStats(Command):
    ...
    def __calc_session_standings_diagram(self):
        self.Verbosity.print("session standing diagrams")

        # diagram output path
        path_dir = os.path.join(self.getArg('http-path-acs-content'), "session_standing_diagrams")
        self.mkdirs(path_dir)
        self.change_group(path_dir)
        ...
        for row_session in self.__db.fetch("Sessions", ['Id', 'Type'], {"Type":3}):
            ...
            for row_laps in self.__db.fetch("Laps", ['User'], {'Session': session_id}):
                ...
                # determine amount of driven laps for this user
                driver_laps = 0
                if driver_id in driver_laps_dict:
                    driver_laps = driver_laps_dict[driver_id]
                driver_laps += 1
                driver_laps_dict.update({driver_id: driver_laps})

                # grow lap standings array
                while len(lap_standings) <= driver_laps:
                    lap_standings.append([])

                # append driver to lap standing
                lap_standings[driver_laps].append(driver_id)
            ...
            # plot all drivers
            for driver_id in driver_laps_dict.keys():
                ...
                for lap_nr in range(1, driver_laps_dict[driver_id] + 1):
                    ...
                    if driver_id in standings:
                        position = 1 + standings.index(driver_id)
                        lap_positions.append(position)
                        lap_numbers.append(lap_nr)

                # plot
                lo = ax.plot(lap_numbers, lap_positions, label=user_login_dict[driver_id])
                fig_line_objcets.append(lo[0])
                fig_driver_labels.append(user_login_dict[driver_id])

            ax.set(xlabel='Race Lap', ylabel='Race Position', title="Race Positions")
            ax.invert_yaxis()

            ax.grid()
            #ax.set_ylim(top=0, bottom=(-0.001*FILTER_BAD_LAPS_DEVIATION))
            fig.legend(fig_line_objcets, labels=fig_driver_labels, loc="center right", borderaxespad=0.1, title="Drivers")
            matplotlib.pyplot.subplots_adjust(right=0.75)
            #fig.set_size_inches(9, 3)
